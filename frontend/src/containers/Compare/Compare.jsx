...
const Compare = () => {
    ...
    const handleCompare = async()=>{
        ...
        try{
            setIsLoading(true)
            const {data:timeSeries} = await axios.get(`https://stockify-backend-q52a.onrender.com/stocks/compare?ticker1=${ticker1}&ticker2=${ticker2}`)
            setDates(extractDates(timeSeries.data[0]))
            setSeries([extractOpen(timeSeries.data[0]),extractOpen(timeSeries.data[1])])
            const {data:stat1} = await axios.get(`https://stockify-backend-q52a.onrender.com/stocks/search?ticker=${ticker1}`)
            const {data:stat2} = await axios.get(`https://stockify-backend-q52a.onrender.com/stocks/search?ticker=${ticker2}`)
